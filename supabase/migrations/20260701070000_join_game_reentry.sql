-- Allow an existing player to re-enter a game that has already started (e.g. they
-- closed the tab or lost signal mid-game and re-join by code). Only NEW players
-- are gated to the lobby; a current member is let back in regardless of phase.
...
begin
  ...
  -- Already a member? Let them back in regardless of phase (mid-game re-entry).
  ...
  -- New player: only joinable while still in the lobby, and only up to capacity.
